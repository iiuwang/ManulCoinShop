...
def register(data: RegisterRequest):
    with get_connection() as conn:
        try:
            cursor = conn.execute(
                """
                INSERT INTO users (login, password, name, balance)
                VALUES (?, ?, ?, 500)
                """,
                (data.login, data.password, data.name),
            )
...
def top_up(data: TopUpRequest, x_user_id: Optional[str] = Header(default=None, alias="X-User-Id")):
    user_id = get_user_id(x_user_id)
    with get_connection() as conn:
        row = conn.execute("SELECT id FROM users WHERE id = ?", (user_id,)).fetchone()
        if not row:
            error(status.HTTP_404_NOT_FOUND, "USER_NOT_FOUND")
        conn.execute("UPDATE users SET balance = balance + ? WHERE id = ?", (data.amount, user_id))
